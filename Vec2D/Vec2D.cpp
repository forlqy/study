...
Vec2D Vec2D::operator+ (const double numeral) {//并非对象拷贝，只是一个简单的值拷贝，所以没必要用引用
...
Vec2D Vec2D::operator* (double multiplier) {//成员函数只适用于向量 * 数，而不能反过来，数在前无法调用operator* (double multipiler)函数
	return this->multiply(multiplier);
}

Vec2D operator* (double multipier, Vec2D vec2d) {//实现友元函数时friend关键字要去掉
	return vec2d.multiply(multipier);
}
// 向量求负值
...
Vec2D& Vec2D::operator++ () {//对于成员函数，前置++、--的参数为空，而后置++、--则要有参数dummy，不使用，只是一个占位符
	x_++; y_++;
	return (*this);
}
...
double& Vec2D::operator[] (const int& index) {
	return at(index);//返回值均为引用类型的传递不会出现问题
}
// 求向量的范数（长度）
